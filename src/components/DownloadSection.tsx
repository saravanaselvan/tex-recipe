...
const DownloadSection: React.FC = () => {
  const { chemicals, totalCostText } = useChemicalContext();
  ...
  return (
    <>
      ...
      <Flex direction={{ base: 'column', sm: 'row' }} gap={{ base: 'sm', sm: 'lg' }}>
        ...
        />
        <Flex gap={8}>
          <Button onClick={() => handleDownload('excel')} disabled={!companyName || !clientName}>
            Download Excel
          </Button>
          <Button onClick={() => handleDownload('pdf')} disabled={!companyName || !clientName}>
            Download PDF
          </Button>
        </Flex>
      </Flex>
